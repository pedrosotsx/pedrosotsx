<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1a2e,100:4f46e5&height=200&section=header&text=Pedro%20Vieira&fontSize=52&fontColor=ffffff&fontAlignY=38&desc=Java%20Back-end%20Developer&descAlignY=58&descSize=20&descColor=a5b4fc" />

</div>

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Pedro%20Vieira-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/pedro-vieira-b98909210)
[![GitHub](https://img.shields.io/badge/GitHub-pedrosotsx-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/pedrosotsx)
[![Email](https://img.shields.io/badge/Email-Contato-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:pedroso.devce@gmail.com)

</div>

---

## Sobre mim

Desenvolvedor **Java Back-end** com foco em construir aplicações organizadas, escaláveis e de fácil manutenção. Aplico na prática princípios de **POO, SOLID, Clean Code e Clean Architecture**, sempre buscando código que seja legível tanto para humanos quanto para o próximo desenvolvedor que vai mantê-lo.

Atualmente focado em solidificar minha base em **APIs REST, banco de dados relacional e arquitetura de software**, evoluindo projeto a projeto.

---

## Stack

### Linguagem principal

<div align="center">
  <img src="https://skillicons.dev/icons?i=java" height="48" alt="Java" />
</div>

<div align="center">

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![POO](https://img.shields.io/badge/POO-1a1a2e?style=flat-square)
![SOLID](https://img.shields.io/badge/SOLID-4f46e5?style=flat-square)
![Clean Code](https://img.shields.io/badge/Clean%20Code-6366f1?style=flat-square)
![Clean Architecture](https://img.shields.io/badge/Clean%20Architecture-818cf8?style=flat-square)
![REST API](https://img.shields.io/badge/REST%20API-02569B?style=flat-square)
![Streams](https://img.shields.io/badge/Streams%20%26%20Collections-ED8B00?style=flat-square&logo=openjdk&logoColor=white)

</div>

---

### Banco de dados

<div align="center">
  <img src="https://skillicons.dev/icons?i=postgresql" height="48" alt="PostgreSQL" />
</div>

<div align="center">

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Modelagem](https://img.shields.io/badge/Modelagem%20de%20Dados-1a1a2e?style=flat-square)
![Joins](https://img.shields.io/badge/Joins%20%26%20Relacionamentos-4f46e5?style=flat-square)

</div>

---

### Ferramentas e versionamento

<div align="center">
  <img src="https://skillicons.dev/icons?i=git,github,vscode" height="48" alt="Ferramentas" />
</div>

<div align="center">

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)
![Code Review](https://img.shields.io/badge/Code%20Review-4f46e5?style=flat-square)

</div>

---

## GitHub Stats

<div align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=pedrosotsx&theme=tokyonight" width="100%" />
</div>

<div align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=pedrosotsx&theme=tokyonight" height="160" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=pedrosotsx&theme=tokyonight" height="160" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=pedrosotsx&theme=tokyonight" height="160" />

---

## Streak & Linguagens

<div align="center">
  <img src="https://streak-stats.demolab.com?user=pedrosotsx&theme=tokyonight&hide_border=true&background=0d1117&stroke=4f46e5&ring=818cf8&fire=a5b4fc&currStreakLabel=a5b4fc&sideLabels=e2e8f0&currStreakNum=ffffff&sideNums=e2e8f0&dates=64748b" />
</div>

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=pedrosotsx&layout=donut&theme=tokyonight&hide_border=true&langs_count=6&title_color=a5b4fc&text_color=e2e8f0&bg_color=0d1117" height="200" />
</div>

---

## Troféus

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=pedrosotsx&theme=tokyonight&no-frame=true&no-bg=true&row=1&column=7" width="100%" />
</div>

---

## Contribuições

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=pedrosotsx&theme=tokyo-night&hide_border=true&bg_color=0d1117&color=a5b4fc&line=4f46e5&point=818cf8" width="100%" />
</div>

---

## Projetos

> Em construção — novos projetos em breve.

---

## Processo de desenvolvimento

```text
Entender  →  Planejar  →  Desenvolver  →  Testar  →  Code Review  →  Refatorar  →  Entregar
```

Utilizo **IA como ferramenta de apoio** — para pesquisa, análise de problemas e revisão de código.
Não substitui o entendimento: todo código é revisado, compreendido e validado antes de ser utilizado.

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:4f46e5,100:1a1a2e&height=120&section=footer&text=Construindo.%20Estudando.%20Evoluindo.&fontSize=18&fontColor=a5b4fc&fontAlignY=65" />

</div>
